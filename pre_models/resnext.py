...
class ResNeXt101_32x4d(nn.Module):
    ...
    def __init__(self, num_classes=1000):
        ...
        elf.my_pool = nn.AdaptiveAvgPool2d(1)
        self.my_linear = nn.Linear(2048, num_classes)

    def logits(self, input):
        x = self.my_pool(input)
        x = x.view(x.size(0), -1)
        x = self.my_linear(x)
        return x
...
class ResNeXt101_64x4d(nn.Module):
    ...
    def __init__(self, num_classes=1000):
        ...
        self.avg_pool = nn.AvgPool2d((7, 7), (1, 1))
        self.last_linear = nn.Linear(2048, 1000)
        self.my_pool = nn.AdaptiveAvgPool2d(1)
        self.my_linear = nn.Linear(2048, num_classes)
...
def resnext101_32x4d(num_classes=1000, pretrained='imagenet'):
    ...
    if pretrained is not None:
        ...
        model.load_state_dict(model_zoo.load_url(settings['url'], model_dir='.'), strict=False)
